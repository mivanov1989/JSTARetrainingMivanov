...
function randArray(n) {
    return Array.apply(null, Array(n)).map((el, index) => Math.floor(Math.random()*500));
}
...
console.log(`Number ${number} raised to degree ${degree} equals ${raiseToDegree(number, degree)}`)
...
console.log(`Minimum number among the arguments is ${findMin(1, -5, 8, 99, -6, 2.3, 84, 0, 0, 0.5)}`);
...
function lastElement(arr, n) {
    let resultArray = [];
    for(let i = Array.from(arr).length - 1; i >= Array.from(arr).length - n; i--) {
        console.log('inside')
...
function capitalizeIt(str) {
   return String(str).trim().split(" ").map(String.chatAt(0).toUpperString());
}
